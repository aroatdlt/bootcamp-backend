...
const winnerByYear = (arr, year) => {
  const filterArrayByYear = arr.filter(item => item.year === year);
  const orderedArray = filterArrayByYear.sort((a,b) => b.ranking - a.ranking).slice(0,3)
  return orderedArray.map(item => item.name)
};
